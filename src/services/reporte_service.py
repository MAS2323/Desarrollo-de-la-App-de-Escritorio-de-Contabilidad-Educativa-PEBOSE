...
from models.base import SessionLocal  # Fix: SessionLocal desde base.py
...
def generar_reporte_pdf(db: Session, filename: str = '../../balance_pebose.pdf'):
...
def generar_reporte_excel(db: Session, filename: str = '../../reporte_pebose.xlsx'):
    data = calcular_balance(db)
    df = pd.DataFrame([data])
    df.to_excel(filename, index=False)
    return filename

def generar_grafico_balance(db: Session, filename: str = '../../grafico_balance.png'):
    data = calcular_balance(db)
